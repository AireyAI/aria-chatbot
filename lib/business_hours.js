...
export function evaluateSchedule(schedule, ts) {
  ...
  // `businessHours` is the canonical engine key (also passed directly by the voice
  // path). The dashboard hours card saves the same map under `days` — accept it as
  // a fallback so owner-saved schedules actually reach the gate. businessHours wins
  // when both are present.
  const hours = sched.businessHours || sched.days || defaultHours;
  ...
  return {
    ...
    // Canonical key is outOfHoursMode; the dashboard saves it as outOfHours.
    outOfHoursMode: sched.outOfHoursMode || sched.outOfHours || 'auto_reply',
