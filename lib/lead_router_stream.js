...
export async function streamRouteChat({
  ...
  for (let iter = 0; iter < MAX_ITERS; iter++) {
    ...
    stream.on('text', t => { try { onTextDelta?.(t); } catch {} });

    // Wait for this turn to complete (so we can inspect tool_use blocks).
    ...
    if (toolUses.length === 0) {
      ...
      return { stopReason: final.stop_reason, toolEvents, score };
    }
