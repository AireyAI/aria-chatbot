...
// ──────────────────────────────────────────────────────────────────────────
// schedule_callback — book a callback at a specific time via serverFns
// (scheduleCallback in server.js → outbound scheduler). The owner is reminded
// at the due time. Owner-facing alert, no approval (same as request_callback).
// ──────────────────────────────────────────────────────────────────────────
export async function handle_schedule_callback(input, ctx) {
  if (!input?.phone || !input?.when) return { error: 'Need the visitor\'s phone number and the time they want the call.' };
  const ownerEmail = ctx?.clientConfig?.handoffEmail || ctx?.clientConfig?.ownerEmail;
  if (!ownerEmail || !ctx?.serverFns?.scheduleCallback) {
    return { scheduled: false, message: 'Scheduled callbacks are not set up for this site — take their number and the team will call back.' };
  }
  try {
    const r = await ctx.serverFns.scheduleCallback({
      ownerEmail, phone: input.phone, name: input.name || null, notes: input.notes || '',
      when: input.when, siteName: ctx?.clientConfig?.businessName || ctx?.clientConfig?.slug,
    });
    if (!r?.scheduled) return { scheduled: false, message: r?.message || 'Could not understand that time — ask for a clearer day and time.' };
    return { scheduled: true, scheduled_for: r.scheduledForHuman };
  } catch (e) {
    console.warn('[aria/schedule_callback] failed:', e?.message || e);
    return { scheduled: false, message: 'Could not schedule the callback right now.' };
  }
}

// ──────────────────────────────────────────────────────────────────────────
// estimate_quote — read-only ballpark from the owner's price list, which the
// widget sends in clientConfig.pricing (same pattern as canned). No server call.
// Aria works out a rough total in prose; we just hand her the matched rule.
// ──────────────────────────────────────────────────────────────────────────
export async function handle_estimate_quote(input, ctx) {
  const pricing = ctx?.clientConfig?.pricing || {};
  const services = Object.keys(pricing);
  if (!services.length) {
    return { available: false, message: 'No price list is configured — gather what they need and use request_quote so the owner can price it.' };
  }
  const norm = s => String(s).toLowerCase().replace(/[^a-z0-9 ]+/g, ' ').replace(/\s+/g, ' ').trim();
  const w = norm(input?.service);
  const wWords = w.split(' ').filter(x => x.length >= 4);
  const match = services.find(s => norm(s) === w)                                              // exact
    || (w && services.find(s => { const sn = norm(s); return sn.includes(w) || w.includes(sn); })) // substring either way
    || (w && services.find(s => norm(s).split(' ').filter(x => x.length >= 4)                  // word overlap (plurals/partials)
         .some(sw => wWords.some(ww => sw.includes(ww) || ww.includes(sw)))));
  if (!match) {
    return { available: true, matched: null, services, message: `No exact match. Priced services: ${services.join(', ')}. Ask which they mean, or use request_quote.` };
  }
  return {
    available: true, service: match, pricing: pricing[match],
    note: 'Give a ballpark from this rule (do the arithmetic in your reply). Say clearly it is an estimate and offer request_quote for an exact figure.',
  };
}

// ──────────────────────────────────────────────────────────────────────────
// take_deposit — share the client's OWN Stripe Payment Link (clientConfig
// .depositUrl). Aria never touches money or keys; deposits go straight to the
// client. Read-only. Graceful when no link is configured.
// ──────────────────────────────────────────────────────────────────────────
export async function handle_take_deposit(_input, ctx) {
  const url = ctx?.clientConfig?.depositUrl;
  if (!url) {
    return { available: false, message: 'No deposit link is set up for this business — do NOT invent one. Continue with the booking; the team will arrange any payment.' };
  }
  return {
    available: true,
    deposit_url: url,
    info: ctx?.clientConfig?.depositInfo || 'a deposit to secure the booking',
    note: 'Share this exact link with the visitor as a clickable link so they can pay. Do NOT state an amount beyond what info says.',
  };
}
...
export const HANDLERS = {
  ...
  estimate_quote:         handle_estimate_quote,
  take_deposit:           handle_take_deposit,
};
