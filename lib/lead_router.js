...
export async function routeChat({
  ...
  // Sum usage across every iteration of the tool-use loop. A multi-turn
  // qualify→lookup→record sequence can easily hit 4-5x the tokens of a single
  // shot, so trackUsage needs the total — not just the last turn.
  ...
  for (let iter = 0; iter < MAX_ITERS; iter++) {
    ...
    if (toolUses.length === 0) {
      ...
      return { reply, toolEvents, stopReason: resp.stop_reason, usage };
    }
