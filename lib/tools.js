...
export const ARIA_TOOLS = [
  ...
  // ── Social proof (2026-06-22) — read-only: cite the business's real reviews.
  {
    name: 'show_social_proof',
    description:
      'Get the business\'s real customer-reviews summary (average rating, total count, a few recent highlights) so you can reassure a hesitating visitor with genuine proof ' +
      '(e.g. "we\'re rated 4.9 from 130 reviews — one customer said ..."). Read-only. ' +
      'Use when the visitor is weighing you up or asks "are you any good / do you have reviews / can I trust you".',
    input_schema: {
      type: 'object',
      properties: {},
    },
  ...
  // ── Scheduled callback (2026-06-22) — book a callback at a specific time;
  // the owner is reminded then via the outbound scheduler. ───────────────────
  {
    name: 'schedule_callback',
    description:
      'Schedule a callback at a SPECIFIC time the visitor asks for (e.g. "call me tomorrow at 2pm") — the business owner is reminded at that time to call them. ' +
      'Also use this when you are currently OUT OF HOURS and the visitor wants a call: offer the next working morning. ' +
      'For an immediate "call me back ASAP" with no specific time, use request_callback instead. Needs their phone number and the time they want.',
    input_schema: {
      type: 'object',
      properties: {
        phone: { type: 'string', description: 'The visitor\'s phone number — required.' },
        when:  { type: 'string', description: 'When they want the call, in their words (e.g. "tomorrow 2pm", "Monday at 9").' },
        name:  { type: 'string' },
        notes: { type: 'string', description: 'What the call is about.' },
      },
      required: ['phone', 'when'],
    },
  },

  // ── Instant quote (2026-06-22) — read-only ballpark from the owner's price
  // list (clientConfig.pricing, like canned). Aria does the arithmetic in prose.
  {
    name: 'estimate_quote',
    description:
      'Give the visitor an instant BALLPARK price for a service from the business\'s configured price list. Use when they ask "how much / what does it cost / rough price?". ' +
      'Returns the pricing rule (from-price, unit, minimum); work out a rough total in your reply from what they\'ve told you, make CLEAR it is an estimate, and offer request_quote for an exact figure. ' +
      'If there is no price list or no match, do NOT guess — use request_quote instead.',
    input_schema: {
      type: 'object',
      properties: {
        service: { type: 'string', description: 'The service they want priced, e.g. "window cleaning".' },
      },
      required: ['service'],
    },
  },

  // ── Take deposit (2026-06-22) — shares the client's OWN Stripe Payment Link
  // (clientConfig.depositUrl). Aria never handles money/keys; deposits go
  // straight to the client. Read-only, no approval.
  {
    name: 'take_deposit',
    description:
      'Share the business\'s secure deposit/payment link so the visitor can pay a booking deposit and lock in their slot. Use once they have decided to book and a deposit is appropriate. ' +
      'Returns the business\'s own payment link — share it as a clickable link. NEVER invent a link or an amount; if none is configured, just continue and let the team arrange payment.',
    input_schema: {
      type: 'object',
      properties: {},
    },
...
export const TOOL_METADATA = {
  ...
  show_social_proof:      { requiresOwnerApproval: false, isJudgment: false },
  schedule_callback:      { requiresOwnerApproval: false, isJudgment: false },
  estimate_quote:         { requiresOwnerApproval: false, isJudgment: false },
  take_deposit:           { requiresOwnerApproval: false, isJudgment: false },
};
